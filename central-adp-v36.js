...
(()=>{
  ...
  async function loadPlayerDirectory(client){
    ...
    return all.filter(r=>['QB','RB','WR','TE'].includes(String(r.position||'').toUpperCase())&&r.player_id&&r.full_name);
  }
  ...
  function reconcileCurrentRankings(rows,directory=activeDirectory){
    ...
    try{
      ...
      // Auto-expansion is only for full ranking boards. Small/custom lists still get identity/team repairs without being filled up.
  ...
  function applyRows(rows,directory=activeDirectory){
    ...
    try{
      ...
      for(const r of directory||[]){
        ...
        dirById.set(id,r);
        target[name]={id,rank:null,posRank:null,team:teamText(r.team),pos,adp:null,searchRank:null,move:null,updatedAt:r.updated_at?new Date(r.updated_at).getTime():Date.now(),central:true,identityOnly:true,format:activeFormat};
      }
      const pool=[];
      for(const r of rows){
        ...
        const entry={id,rank:Number(r.sleeper_rank),posRank:Number(r.position_rank),team,pos,adp:Number(r.sleeper_adp),searchRank:Number(r.sleeper_adp),move:Number(r.rank_change)||0,updatedAt:r.captured_at?new Date(r.captured_at).getTime():Date.now(),central:true,identityOnly:false,format:activeFormat};
        target[name]=entry;
  ...
  async function loadCentralRanks(){
    ...
    try{
      ...
      const adpPromise=client.from('sleeper_adp_current')
      ...
      const [adpResult,directory]=await Promise.all([
        adpPromise,
        loadPlayerDirectory(client).catch(e=>{console.warn('Sleeper player directory unavailable',e);return []})
      ]);
      const {data,error}=adpResult;if(error)throw error;
      if(!Array.isArray(data)||data.length<100)throw new Error('Shared Sleeper '+FORMATS[activeFormat].short+' ranks are incomplete.');
      activeRows=data;activeDirectory=directory;applyRows(activeRows,activeDirectory);
  ...
  [1200,3000,6500].forEach(ms=>setTimeout(()=>{if(activeRows.length){applyRows(activeRows,activeDirectory);if(typeof renderEverything==='function')renderEverything()}},ms));
})();
