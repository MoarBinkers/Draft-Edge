...
(()=>{
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='deForgot56Css';s.textContent=`
      ...
      #deForgot56 .send{width:100%;margin-top:12px;padding:13px 16px;border:1px solid #58a9e1;border-radius:11px;background:linear-gradient(135deg,#2588ca,#17649d);color:#fff;font-weight:950;font-size:13px;cursor:pointer;box-shadow:0 10px 25px rgba(14,94,151,.24);transition:transform .18s,filter .18s,box-shadow .18s}
      #deForgot56 .send:hover{transform:translateY(-2px);filter:brightness(1.08);box-shadow:0 14px 30px rgba(14,94,151,.34)}
      #deForgot56 .send:active{transform:translateY(0) scale(.99)}
      #deForgot56 button:disabled,#deForgot56 input:disabled{opacity:.58;cursor:not-allowed;transform:none;filter:none}
  ...
  function ensure(){
    ...
    r.innerHTML=`<div class="card"><div class="head"><div><h2>Reset your password</h2><div class="sub">Enter the email on your Draft Edge account. We’ll send you a link to choose a new password.</div></div><button class="close" id="deForgotClose56" aria-label="Close">×</button></div><input id="deForgotEmail56" type="email" inputmode="email" autocomplete="email" placeholder="Email address"><button class="send" id="deForgotSend56">Send Recovery Email</button><div class="msg" id="deForgotMsg56"></div><button class="done" id="deForgotDone56">Done</button></div>`;
  ...
  function open(){
    const r=ensure();r.classList.remove('sent','limited');message('');const input=$('deForgotEmail56');const prior=$('authEmail')?.value?.trim();if(input){if(prior)input.value=prior;input.disabled=false}const b=$('deForgotSend56');if(b){b.disabled=false;b.textContent='Send Recovery Email'}r.classList.add('open');setTimeout(()=>input?.focus(),30);
  }
  function close(){if(sending)return;$('deForgot56')?.classList.remove('open')}

  async function send(){
    if(sending)return;const input=$('deForgotEmail56');const email=(input?.value||'').trim();
    ...
    sending=true;const b=$('deForgotSend56');if(input)input.disabled=true;if(b){b.disabled=true;b.textContent='Sending…'}message('Sending your recovery email…');
    try{
      const {error}=await supabaseClient.auth.resetPasswordForEmail(email,{redirectTo:LIVE_URL});if(error)throw error;
      if($('authEmail'))$('authEmail').value=email;$('deForgot56')?.classList.add('sent');message('Email sent. Open the newest Draft Edge recovery email, then use the link to choose your new password.','ok');if(b)b.textContent='Recovery Email Sent';
    }catch(e){
      const raw=String(e?.message||'Could not send the recovery email.');
      const seconds=raw.match(/after\s+(\d+)\s+seconds?/i);
      const broad=/email rate limit exceeded/i.test(raw);
      if(seconds){
        message(`A recovery email was requested very recently. Wait about ${seconds[1]} seconds, then try again.`,'err');
        if(input)input.disabled=false;if(b){b.disabled=false;b.textContent='Send Recovery Email'}
      }else if(broad){
        $('deForgot56')?.classList.add('limited');
        message('Draft Edge’s email service has reached its sending limit. No new email was sent. Use the newest recovery email already in your inbox, or try again later.','err');
        if(b){b.disabled=true;b.textContent='Email Limit Reached'}
      }else{
        message(raw,'err');if(input)input.disabled=false;if(b){b.disabled=false;b.textContent='Send Recovery Email'}
      }
    }finally{sending=false}
  }

  // Retire v55 if it was already created, and capture Forgot Password before older handlers.
  $('deForgot55')?.remove();
